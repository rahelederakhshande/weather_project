...
def setup():
    ...
    parser.add_argument("--city",type=str,help="Get Name")
    parser.add_argument("--forecast",action="store_true",help="forecast")
    parser.add_argument("--details",action="store_true",help="display details")
    parser.add_argument("--show_logs",action="store_true",help="show all logs")
    return parser
...
if args.all:
    ...
    if name not in data:
        ...
        print(f"Current condition in {name}: {data[name]['current_condition']}") 
elif args.show_logs:
    show_logs()        
